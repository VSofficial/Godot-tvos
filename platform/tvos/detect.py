...
def get_opts():
    ...
    return [
        ...
        (("APPLE_TOOLCHAIN_PATH", "TVOS_TOOLCHAIN_PATH"), "Path to the Apple toolchain", ""),
        (("APPLE_SDK_PATH", "TVOS_SDK_PATH"), "Path to the tvOS SDK", ""),
        (("apple_target_triple", "tvos_triple"), "Triple for the corresponding target Apple platform toolchain", ""),
        BoolVariable(("simulator", "tvos_simulator"), "Build for Simulator", False),
...
def configure(env: "SConsEnvironment"):
    ...
    if env["simulator"]:
        ...
        env.Append(ASFLAGS=["-mtvos-simulator-version-min=14.0"])
        env.Append(CCFLAGS=["-mtvos-simulator-version-min=14.0"])
        env.Append(CPPDEFINES=["TVOS_SIMULATOR"])
        env.extra_suffix = ".simulator" + env.extra_suffix
    else:
        env["APPLE_PLATFORM"] = "tvos"
        env.Append(ASFLAGS=["-mtvos-version-min=14.0"])
        env.Append(CCFLAGS=["-mtvos-version-min=14.0"])
    detect_darwin_sdk_path(env["APPLE_PLATFORM"], env)

    if env["arch"] == "x86_64":
        if not env["simulator"]:
            print_error("Building for tvOS with 'arch=x86_64' requires 'simulator=yes'.")
            sys.exit(255)
